 ...
 * e.g. Delay = 10 μsec => Baud Rate < 100 kHz */
...
s2pi_handle_t s2pi_ = {
    .SpiAlternate = GPIO_AF0_SPI1,
    .GPIOs = {
        [ S2PI_CLK ] = { GPIOB, GPIO_PIN_3 },
        [ S2PI_CS ] = { GPIOB, GPIO_PIN_4 },
        [ S2PI_MOSI ] = { GPIOA, GPIO_PIN_12 },
        [ S2PI_MISO ] = { GPIOA, GPIO_PIN_11 },
        [ S2PI_IRQ ] = { GPIOC, GPIO_PIN_6 }
    }
};
...
status_t S2PI_TransferFrame(
  ...
  if (rxData)
    hal_error = HAL_SPI_TransmitReceive_DMA(&hspi1, (uint8_t*) txData, rxData,
        (uint16_t) frameSize);
  else
    hal_error = HAL_SPI_Transmit_DMA(&hspi1, (uint8_t*) txData,
        (uint16_t) frameSize);
